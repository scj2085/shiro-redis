package com.gome.meidian.shiroredis.shiro.db;
...
public class JedisShiroCache<K, V> implements Cache<K, V> {
    ...
    public JedisShiroCache(String name, JedisUtil jedisUtil) {
        this.name = name;
    ...
    public V get(K key) throws CacheException {
        ...
        try {
            byteValue = jedisUtil.getValueByKey(DB_INDEX, byteKey);
        } catch (Exception e) {
    ...
    public V put(K key, V value) throws CacheException {
        V previos = get(key);
        try {
            jedisUtil.saveValueByKey(DB_INDEX, SerializeUtil.serialize(buildCacheKey(key)),
    ...
    public V remove(K key) throws CacheException {
        V previos = get(key);
        try {
            jedisUtil.deleteByKey(DB_INDEX, SerializeUtil.serialize(buildCacheKey(key)));
        } catch (Exception e) {
